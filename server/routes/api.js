...
// ---------------------------------------------------------------------------
// ИМЯ КРЕАТОРА — при первом входе в кабинет креатор вместо ника в Telegram
// (он остаётся служебным полем, виден только админу) выбирает отображаемое
// имя, под которым его видят остальные креаторы (топы, лидерборды). Как и
// прочие «личные» действия в этом прототипе (разметка видео, вывод средств
// и т.д.) — применяется к MYNICK, единственному креатору, за которого можно
// реально «зайти» в кабинет.
// ---------------------------------------------------------------------------
router.post('/creators/me/name', (req, res) => {
  const raw = String((req.body && req.body.name) || '').trim();
  if (!raw) return res.status(400).json({ error: 'Введите имя' });
  if (raw.length > 40) return res.status(400).json({ error: 'Имя слишком длинное — до 40 символов' });
  if (store.isNameTaken(raw, store.MYNICK)) {
    return res.status(409).json({ error: 'Это имя уже занято — выбери другое' });
  }
  const me = store.CREATORS.find((c) => c.n === store.MYNICK);
  if (!me) return res.status(404).json({ error: 'Креатор не найден' });
  me.name = raw;
  res.json({ creators: store.CREATORS, me });
});
